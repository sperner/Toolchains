#!/bin/bash
#
# generate_avr-toolchain.sh 2014-03-17
# Implemented & tested with >Gentoo< linux. Should also work on other distros
#
# Patches are from:
# http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-binutils/files/
# http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-gcc/files/
# http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-libc/files/
# http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-gdb/files/
#
...
PATCHES_BINUTILS="patch-300-binutils-2.20.1-avr-size patch-301-binutils-2.20.1-avr-coff patch-302-binutils-2.20.1-new-sections patch-303-binutils-2.20.1-as-dwarf patch-304-binutils-2.20.1-dwarf2-AVRStudio-workaround patch-305-binutils-2.20.1-assembler-options patch-400-binutils-2.20.1-xmega patch-401-binutils-2.20.1-new-devices patch-402-binutils-2.20.1-avrtiny10 patch-403-binutils-2.20.1-xmega128a1u-64a1u patch-404-binutils-2.20.1-atxmega16x1-32x1 patch-405-binutils-2.20.1-atxmega128b1  patch-406-binutils-2.20.1-atxmega256a3bu patch-407-binutils-2.20.1-at90pwm161 patch-408-binutils-2.20.1-atmega16hvb-32hvb patch-409-binutils-2.20.1-atmega32_5_50_90_pa patch-410-binutils-2.20.1-attiny1634 patch-411-binutils-2.20.1-atmega48pa patch-500-binutils-2.20.1-bug13789 patch-Makefile.in"
PATCHES_GCC="patch-200-gcc-4.5.1-libiberty-Makefile.in patch-300-gcc-4.5.1-fixedpoint-3-4-2010 patch-301-gcc-4.5.1-xmega-v14 patch-302-gcc-4.5.1-avrtiny10 patch-303-gcc-4.5.1-osmain patch-304-gcc-4.5.1-builtins-v6 patch-305-gcc-4.5.1-avrtiny10-non-fixedpoint patch-306-gcc-4.5.1-option-list-devices patch-400-gcc-4.5.1-new-devices patch-401-gcc-4.5.1-atmega32_5_50_90_pa patch-402-gcc-4.5.1-attiny1634 patch-403-gcc-4.5.1-atmega48pa patch-500-gcc-4.5.1-bug13473 patch-501-gcc-4.5.1-bug13579 patch-502-gcc-4.5.1-bug-18145-v4 patch-503-gcc-4.5.1-avrtiny10-bug-12510 patch-504-gcc-4.5.1-bug12915 patch-505-gcc-4.5.1-bug13932 patch-506-gcc-4.5.1-bug13789 patch-599-gcc-4.5.1-bug46779 patch-gcc-config-avr-driver-avr.c patch-plugins"
PATCHES_GDB="patch-bfd-bfdio.c patch-bfd-elflink.c patch-bug13519"
...
install_binutils()
{
	...
	for p in $PATCHES_BINUTILS ; do
		test -r "$p" || (wget "http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-binutils/files/$p" || exit 1)
	done
	cd ..
...
install_gcc()
{
	...
	for p in $PATCHES_GCC ; do
		test -r "$p" || (wget "http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-gcc/files/$p" || exit 1)
	done
	cd ..
...
install_libc()
{
	...
	echo -e $CDOW"Checking for avr-libc-manpages"$CS
	test -r "avr-libc-$AVRLIBC.tar.bz2"  || (echo -e "Downloading avr-libc-manpages... " ; wget "http://download.savannah.gnu.org/releases/avr-libc/avr-libc-manpages-$AVRLIBC.tar.bz2" || exit 1 ; echo "ok")
	...
	su -c "export PATH=$PATH:$CONFIG_PREFIX/bin; make install && tar -xjvf ../../avr-libc-manpages-$AVRLIBC.tar.bz2 -C $CONFIG_PREFIX" || (
		echo -e $CEND"Wrong password! Please try again"$CS
		su -c "export PATH=$PATH:$CONFIG_PREFIX/bin; make install"		|| exit 1
	)
	cd ../..
...
#		test -r "$p" || (wget "http://raw.github.com/freebsd/freebsd-ports/master/devel/avr-gdb/files/$p" || exit 1)
#	done
#	cd ..
...
install_insight()
{
	if [ $DELETE -eq 1 ] ; then
		echo -e $CDOW"Removing old build-directory"$CS
		test -d "insight-$INSIGHTDIR"  && ( echo -n "removing insight-$INSIGHTDIR...  "   ; rm -rf insight-$INSIGHTDIR  || exit 1 ; echo "ok" )
	fi
